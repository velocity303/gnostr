...
- PostWidget like-toggle icon uses `Gtk.Image.set_from_icon_name()` (NOT `set_icon_name` — that setter does not exist on Gtk.Image and crashed every PostWidget on feed load); the icon is the button's first child, `btn_like.get_child().get_first_child()`
