...
class FeedService:
    ...
    def __init__(self, event_repo):
        ...
        print(
            "INFO: FeedService initialized with dedicated cache manager for rate limiting."
        )

    def get_paginated_global_feed(
        self, current_cursor: Optional[str] = None, page_size: int = 20
    ) -> Dict[str, Any]:
        """
        ...
        try:
            raw_events = self._event_repository.find_paginated_events(
                {"kind": 1, "authors": [], "cursor": current_cursor},  # Global query
                limit=page_size,
            )
            ...
            CACHE.set(
                cache_key,
                {"source": "error", "events": [], "cursor": None},
                ttl_seconds=60,
            )
