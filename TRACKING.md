...
- ⏳ **Task 10 — Video stutter perf investigation.** Measure-first instrumentation shipped (`perf: instrument video pipeline for stutter measurement`): `VideoPlayer._inspect_pipeline` reports the decoder element(s) + negotiated video caps (resolution/fps) per URL, state-change timing and cache evictions are logged — all gated behind `_DEBUG_VIDEO` (off by default, zero production cost). Next step: flip `_DEBUG_VIDEO=True` on-device, collect the `🎬[PERF]` log, then make a targeted tuning commit (resolution cap / frame-drop policy / decoder forcing) — no speculative config until measurements land. Also fixed stale `src/gnostr/AGENTS.md` (VideoPlayer was documented as appsink; it's `gtk4paintablesink` → `Gdk.Paintable` → `Gtk.Picture` now).
