...
- ⏳ **Task 10 — Video stutter perf investigation.** Fifth capture delivered the real signal: **119 `QOS` bus events** (dropped-frame reports, 524288 = `GST_MESSAGE_QOS`) during playback — that is the measured stutter. One real **codec error** on `blossom.band/6f5c6a58...mp4`: *"Video device uses an unsupported pixel format"* + *"Internal data stream error"* (retried twice, lines ~371 & ~808). Root cause: the Task 10 capsfilter caps carry only a width/height range with **no `format` field**, so playbin3's autoplug stops inserting `videoconvert` — the decoder's NV12/I420 hits gtk4paintablesink (RGBA-only) and fails. Fixed in `fix: deterministic format adaptation in capped video sink`: added a `videoconvert` inside the capsfilter bin (capfilter → conv → sink) so format conversion is guaranteed regardless of autoplug. Only the tiny 220x124 video had played cleanly (its format matched by luck) — confirming the regression. The lone real `video_caps=RGBA 220x124` report (line 488) proves the bin-descend measurement now works. Next: rebuild + replay the same blossom.band mp4 to confirm it renders without the codec error.
