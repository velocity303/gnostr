...
- ⏳ **Task 10 — Video stutter perf investigation.** Measurement now fully attributed per-URL: QOS events (stutter) aggregate per-pipeline with `dropped_late` sum + worst `proportion`, emitted as `PERF-QOS {url} events=N dropped_late=M worst_prop=0.xx caps=...` at player eviction; state-changed lines carry the URL; measured caps are stashed for correlation. This turns the bare `Bus: 524288` flood into per-video stutter stats we can chase. Fifth capture showed the codec error is **isolated to blossom-hosted files** (one is the exact same file that failed pre-fix) — broken source, not a regression. Next: capture with the new PERF-QOS summaries, replay several videos, and read the per-URL stutter + caps correlation to pick the next tuning target.
