...
- ⏳ **Task 10 — Video stutter perf investigation.** Second on-device capture: `_inspect_pipeline` now fires (tied to state-changed) and reports per URL, but it exposed two bugs I fixed in `fix: video perf measurement tooling (round 2)`: (1) the bus-handler whitelist referenced `Gst.MessageType.CLOCK_UPDATE`, which doesn't exist in the Flatpak GStreamer bindings → `AttributeError` on every else-reached bus message (the repeated CLI error); now a getattr-safe bitmask. (2) the caps query called `get_current_caps()` on the element — that's a Pad method, so it raised (`video_caps=query-error`); now queries the sink's "sink" pad. Preroll remains fast and error-free; no QOS dropped-frame reports. Next: one more capture to get real decoder+resolution, then the tuning decision (resolution cap / decoder forcing) — no speculative config until measurements land.
