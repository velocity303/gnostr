...
- ⏳ **Task 10 — Video stutter perf investigation.** **Major correction:** the earlier "119/174 QOS events" were mis-read — `Bus: 524288` is **LATENCY** (`1<<19`, benign pipeline noise), not QOS. The real dropped-frame signal is **QOS = `1<<24`** and it was **zero in the capture** (2 events across 47 evictions, `dropped_late=0`). So videos are **not dropping frames** — the aggregation works (those 2 events prove the `QOS` enum exists in bindings and aggregates silently). The perceived "stutter" is **preroll/startup latency**, not frame drops: `1→2` preroll median **3325ms** (worse with more videos), `2→3` median **3325ms**, max **6362ms**, peak **7 concurrent pipelines**. **Fix 1 (b0fa199):** narrowed bus whitelist to only the numeric `QOS` bitmask `1<<24` and dropped LATENCY/SEGMENT_DONE/PROGRESS/CLOCK_UPDATE noise. **Fix 2 (lazy-load):** real videos no longer build a playbin3 pipeline / pull the stream at render time — they show a placeholder "Play video" graphic and build on first play tap (`ContentRenderer._start_playback`), killing the 6+ concurrent-preroll contention; GIFs keep the existing preroll+loop UX. Next: capture should show far fewer pipeline builds + lower preroll, and confirm placeholder→spinner→video handoff.
