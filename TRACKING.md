...
- ⏳ **Task 10 — Video stutter perf investigation.** Third capture delivered real measurements: `🎬[PERF]` now reports per URL — 640x360 YouTube, 224x236 small mp4, 1260x1520 tall 60fps, and **1920x1080 (1080p)** mp4 — full 1080p decoded into a ~600px feed widget. No errors, no QOS dropped-frame reports, preroll fast. Tuning landed (`perf: cap video frame size for mobile`): the sink is wrapped in a capsfilter bin capping delivered frames to `_MAX_VIDEO_WIDTH=1280` x `_MAX_VIDEO_HEIGHT=720` (decodebin3 auto-inserts videoscale; falls back to a direct sink if the bin can't build, so video always works). Next: rebuild + one short capture to confirm the `🎬[PERF]` caps show ≤1280px and video still renders. DOX: `src/gnostr/AGENTS.md` VideoPlayer contract updated.
