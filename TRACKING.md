...
- ⏳ **Task 10 — Video stutter perf investigation.** On-device log analyzed (`~/Briefcase/output.txt`): preroll is fast (104–225ms state transitions, no errors/EOS, no QOS dropped-frame reports) — Python-side build/setup is NOT the stutter source; the sink renders natively. BUT the first `_inspect_pipeline` measurement silently no-op'd (decoder+resolution never captured — print sat after a fragile decoder walk, so any walk failure swallowed the whole report), and the debug log flooded with 1350 `STREAM_START` (32) lines from the bus handler's catch-all `else`. Fix landed (`fix: video perf measurement tooling`): inspect now fires on the guaranteed PAUSED/PLAYING state-changed event (once per pipeline, not a timer), queries+prints caps first so the report can't be lost, and the bus handler whitelists only perf-relevant message types (QOS/LATENCY/SEGMENT_DONE/PROGRESS/CLOCK_UPDATE). Next: one focused re-capture to get decoder + resolution, then the tuning decision (resolution cap / decoder forcing) — no speculative config until measurements land.
