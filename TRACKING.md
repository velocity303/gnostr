...
- ✅ **UX batch 2 (6ad393c).** (1) Clicking a video no longer navigates into the post — the video frame's `Gtk.GestureClick` CLAIMS its sequence on press, denying the enclosing PostWidget's open-thread gesture. (2) Long-press copy lengthened to ~750ms (`GestureLongPress` delay-factor 1.5). (3) Feed dedup by event_id in `on_event_received` (switch_feed DB cache + live re-delivery of persisted events double-rendered). (4) Thread replies render as a reply TREE (parent = last `e` tag per NIP-01), depth-first with indentation, so a reply-to-a-reply nests under its direct parent even if newer; live replies reload the tree and request `#e:[eid]` so deeper nesting fills in. Files: `renderer.py`, `post_widget.py`, `main.py`, `thread_view.py`.
