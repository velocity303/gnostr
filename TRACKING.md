...
- ⏳ **Task 10 — Video stutter perf investigation.** Fourth capture confirmed the cap works: `mGZnUgLEdsg9C9Ra.mp4` reports `video_caps=... width=1280, height=720` (was uncapped), GIF stays 498x280 — the capsfilter bin is scaling to ≤1280x720. Most videos reported `video_caps=none` because with the cap active `video-sink` is a bin and the measurement queried its ghost pad. Fixed in `fix: report capped video caps from the real sink`: `_inspect_pipeline` now descends into the video-sink bin to the gtk4paintablesink (named "sink") and queries its sink pad. Next: one playback capture (actually play the 1080p mp4) to confirm it reports ≤1280px and video renders (`3→4` transitions, no errors) — then Task 10 closes.
